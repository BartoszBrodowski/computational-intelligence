...
# Max fitness = 9*9 + 9*9 + 9*9 + 9*9 = 324
...
def count_elements(lst):
    ...
    for element in lst:
        if element in element_counts:
            ...
            element_counts[element] = 1
    return set(element_counts.items())
...
def fitness_function(solution, solution_idx):
    ...
    occurences_list = count_elements(solution)

    for number in occurences_list:
        if number[1] == 9:
            fitness += 9
